...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        boolean decide = promptForPlayAgain(keyboard);
    ...
    private static String getResult(int[] dice) {
        ...
        while (pos < 4) {
            ...
            sum = sum + dice[pos + 1]; //Extra credit for Straight
            pos++;

        }

        int[] newdice = getCounts(dice);
        pos = 0;
        int largest = newdice[pos];
        while (pos < 4) {
            if (largest < newdice[pos + 1]) {
                largest = newdice[pos + 1];
            }
            pos++;
        }
